...
struct http2_request {
  std::string method;    // ":method" pseudo-header (GET, PUT, POST, DELETE, PATCH)
  std::string path;      // ":path" pseudo-header — path component only (before '?')
  std::string raw_query; // query string portion of :path (after '?', no leading '?')
  std::string scheme;    // ":scheme" pseudo-header
  std::string authority; // ":authority" pseudo-header
  std::map<std::string, std::string> headers; // regular headers (all lowercase)
...
class http2_response {
 public:
  ...
  http2_response(nghttp2_session* session, int32_t stream_id,
...
using http2_handler = std::function<void(const http2_request&, http2_response&)>;
...
class http2_server {
 public:
  http2_server(const std::string& address, uint32_t port,
  ...
  // Route lookup — public so file-scope nghttp2 callbacks (on_frame_recv_callback
  // etc.) in the .cpp can call conn->server->find_handler() without friendship.
